...
static inline void motor_set_direction(uint8_t direction)
{
	if(direction & 1) {
		SET_BIT(PORTC, 3);
	} else {
...
static inline void motor_set_duty(uint16_t duty) // values from 0 to 400
{
	if(duty > 400) {
		duty = 400;
	}
...
static inline void motor_setup()
{
