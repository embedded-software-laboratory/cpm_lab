...
//
// Arguments    : const mgen::driving_Path *refPath
//                double speed
...
namespace mgen
{
                        ...
                        struct1_T, 1U> &trajectory_points)
  {
    ...
    allSegmentLengths.set_size((n + 1));
    for (i = 0; i <= n; i++) {
    ...
    for (nPoses = 0; nPoses <= n; nPoses++) {
      if (isTransPose[nPoses + 1]) {
        ...
        allSegmentLengths[nPoses + 1] = ((static_cast<double>
          (allSegmentLengths.size(0)) + 1.0) - static_cast<double>(lastIndex)) *
          0.5;
      }
    }
    ...
    trajectory_points[0].t = 2000000000UL;
    ...
    for (nm1d2 = 0; nm1d2 <= i - 3; nm1d2++) {
      ...
      //  [m/s]
      kd = allSegmentLengths[nm1d2 + 1] / speed * 1.0E+9;
      ndbl = rt_roundd_snf(kd);
      if (kd >= 0.0) {
        if (kd < 1.8446744073709552E+19) {
          qY = trajectory_points[1].t + static_cast<unsigned long>(ndbl);
          if (qY < trajectory_points[1].t) {
            qY = MAX_uint64_T;
          }

          trajectory_points[nm1d2 + 2].t = qY;
        } else {
          trajectory_points[nm1d2 + 2].t = MAX_uint64_T;
        }
      } else if (kd < 0.0) {
        if (-kd < 1.8446744073709552E+19) {
          if (kd - ndbl == 0.5) {
            qY = trajectory_points[1].t - static_cast<unsigned long>(std::floor(
              -kd));
            if (qY > trajectory_points[1].t) {
              qY = 0UL;
            }
            ...
            qY = trajectory_points[1].t - static_cast<unsigned long>
              (rt_roundd_snf(-kd));
            if (qY > trajectory_points[1].t) {
              qY = 0UL;
            }
          ...
          trajectory_points[nm1d2 + 2].t = 0UL;
        }
      } else {
        trajectory_points[nm1d2 + 2].t = 0UL;
      }
